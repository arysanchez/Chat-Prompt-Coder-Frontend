Para garantir que o conteúdo do prompt seja enviado corretamente para a barra de mensagem, precisamos ajustar a interface `Prompt` para incluir a propriedade `prompt` e garantir que estamos utilizando essa propriedade ao enviar o prompt para a barra de mensagem.

### frontend/src/types.ts [coder:save]
```typescript
...
export interface Prompt {
  ...
  title: string;
  description?: string;
  prompt: string; // Adicionando a propriedade prompt
}
```
[coder:end]
...
Agora, quando apenas um prompt for selecionado, o conteúdo do prompt (`prompt.prompt`) será enviado para a barra de mensagem para o usuário enviar na conversação. Se precisar de mais alguma coisa, por favor, me avise!