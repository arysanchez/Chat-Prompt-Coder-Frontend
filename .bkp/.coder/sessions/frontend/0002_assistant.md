Para corrigir o problema de envio do título do prompt em vez do conteúdo do prompt quando apenas um prompt é selecionado, precisamos ajustar a função `handleSavePrompts` no componente `Sidebar`. Vamos garantir que o conteúdo do prompt seja enviado para a área de conversação.
...
Agora, quando apenas um prompt for selecionado, o conteúdo completo do prompt será enviado para a área de conversação.