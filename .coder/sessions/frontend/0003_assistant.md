Para adaptar o envio da mensagem para a rota `/send-message`, precisamos ajustar o payload que está sendo enviado na requisição. A estrutura do payload deve corresponder ao que a API espera.

Vamos atualizar a função `sendMessage` no arquivo `endpoints.ts` para enviar o payload correto.
