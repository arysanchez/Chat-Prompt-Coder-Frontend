Let's ensure that the response from the `sendMessage` function is correctly handled and that the content is properly set in the state.
...
These updates ensure that the response from the `sendMessage` function is correctly handled and that the content is properly set in the state. Please try this update and let me know if the issue persists.